...
@pytest.mark.parametrize(
    "url,expected_bucket,expected_key,expected_parent,expected_scheme,expected_is_dir", [
        ...
        ("s3://123-bucket/another/key/path", "123-bucket", "another/key/path", S3Path(url="s3://123-bucket/another/key/"), "s3", False),
        ("s3://bucket/this/is/a/folder/", "bucket", "this/is/a/folder/", S3Path(url="s3://bucket/this/is/a/"), "s3", True),
        ("s3://my-bucket/some/key/path/file.txt", "my-bucket", "some/key/path/file.txt", S3Path(url="s3://my-bucket/some/key/path/"), "s3", False),
        ("s3://10-bucket-01/nested.folder/structure/file.jpg", "10-bucket-01", "nested.folder/structure/file.jpg", S3Path(url="s3://10-bucket-01/nested.folder/structure/"), "s3", False),
        ...
        ("s3a://123-bucket/another/key/path", "123-bucket", "another/key/path", S3Path(url="s3a://123-bucket/another/key/"), "s3a", False),
        ("s3a://bucket/this/is/a/folder/", "bucket", "this/is/a/folder/", S3Path(url="s3a://bucket/this/is/a/"), "s3a", True),
        ("s3a://my-bucket/some/key/path/file.txt", "my-bucket", "some/key/path/file.txt", S3Path(url="s3a://my-bucket/some/key/path/"), "s3a", False),
        ("s3a://10-bucket-01/nested.folder/structure/file.jpg", "10-bucket-01", "nested.folder/structure/file.jpg", S3Path(url="s3a://10-bucket-01/nested.folder/structure/"), "s3a", False),
        ("s3a://my-bucket/with-key", "my-bucket", "with-key", S3Path(url="s3a://my-bucket/"), "s3a", False),
    ]
)
def test_good_s3_paths(url: str, expected_bucket: str, expected_key: str, expected_parent: S3Path, expected_scheme: str, expected_is_dir: bool) -> None:
    path = S3Path(url=url)
