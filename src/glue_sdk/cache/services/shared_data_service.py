...
class SharedDataService(ICache,metaclass=SingletonABCMeta):
    ...
    def _chek_type(self,key: str,value:Any, type_assert: Any):
        assert isinstance(value,type_assert), f"Missing: get. {key} have to be type: {type} but {type(value)}"
    ...
    def set(self, key: str, value: Any, timeout: int = 5) -> None:
        ...
        while not lock.acquire(blocking=False):
            if time.time() - start_time > timeout:
                raise TimeoutError(f"Failed to acquire lock for key '{key}' within {timeout} seconds.")
            time.sleep(0.01) 
        try:
