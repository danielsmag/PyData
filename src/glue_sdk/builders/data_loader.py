...
class DataLoader(DataBuilderBase, IDataLoader):
    ...
    @validate_call
    def load(
        self, db_name: str, table_name: str, keep_dynamicframe: bool = False
    ) -> Self:
        """
    ...
    def _load_dynamic_frame(self) -> Self:
        assert isinstance(self.table_name, str) and isinstance(
            self.db_name, str
        ), "table or db is not str"
        assert isinstance(self.data_catalog_service, "IDataCatalogService")
        self.data = self.data_catalog_service.load_dynamic_frame(
            database_name=self.db_name, table_name=self.table_name
        )
        if not self.data:
            error_msg: str = (
                f"Failed to load data from {self.db_name}.{self.table_name}"
            )
