...
class AuroraPgService(IAuroraPgService, BaseService):
    ...
    def __init__(
        ...
        self.connection_name: str = connection_name or self.config.get(
            "connection_name", ""
        )
    ...
    def _get_worker(self, worker_type: str) -> "IAuroraPgWorker":
        ...
        if not worker:
            raise ValueError(
                f"Worker type '{worker_type}' is not initialized or unsupported"
            )
        return worker
    ...
    def fetch_data(
        ...
        return worker.fetch_data(
            table_name=table_name, push_down_predicate=push_down_predicate
        )
