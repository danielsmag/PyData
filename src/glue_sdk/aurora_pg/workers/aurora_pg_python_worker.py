...
class Psycopg2AuroraPgWorker(IAuroraPgWorker, BaseService):
    ...
    def __init__(self, config: Dict, aurora_pg_client: "IAuroraPgClient") -> None:
        """
    ...
    def fetch_data(
        self, table_name: str, push_down_predicate: Optional[str] = None
    ) -> "DataFrame":
        """
        ...
        try:
            ...
            self.log_debug(
                f"Fetching data from table '{table_name}' with predicate: {push_down_predicate}"
            )
            ...
            with self.client.create_client() as conn:
                with conn.cursor() as cursor:
                    ...
                    if not cursor.description:
                        raise Psycopg2AuroraPgWorkerError(
                            "No  cursor.description exist"
                        )
                    columns: List[Any] = [desc[0] for desc in cursor.description]
                    rows = cursor.fetchall()
                    self.log_debug(
                        f"Fetched {len(rows)} rows from table '{table_name}'"
                    )
            ...
            if rows:
                ...
                self.log_debug(
                    f"Successfully converted fetched data to Spark DataFrame for table '{table_name}'"
                )
                return df
            else:
    ...
    def load_data(
        ...
        try:
            ...
            with self.client.create_client() as conn:
                ...
                conn.commit()
                self.log_info(
                    f"Successfully loaded {len(data)} records into table '{table_name}'"
                )
                return True
