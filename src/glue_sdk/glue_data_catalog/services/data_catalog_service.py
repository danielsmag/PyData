...
class DataCatalogService(BaseService, IDataCatalogService):
    ...
    def fetch_table(
        ...
        try:
            ...
            response: Dict = self.glue_client.get_table(
                DatabaseName=db.name, Name=table_name
            )
            ...
            if error_code == "EntityNotFoundException":
                raise ValueError(f"Table {table_name} does not exist in db {db}")
            else:
                raise Exception(f"Error connecting to Glue Table {table_name}: {e}")

    def fetch_db(self, db_name: str) -> "DataCatalogDatabase":
        """get db meta data"""
        try:
    ...
    def load_dynamic_frame(self, database_name: str, table_name: str) -> "DynamicFrame":
        ...
        try:
            ...
            if dyf is None:
                raise ValueError(
                    f"fail to extract data from {database_name}.{table_name}. DynamicFrame is None"
                )
            return dyf
    ...
    def write_iceberg_s3(
        ...
        mode: Literal[
            "overwrite", "ignore", "error", "errorifexists", "append"
        ] = "append",
        ...
        try:
            ...
            self.log_info(
                message=f"Preparing to write DataFrame to Iceberg table: {iceberg_table} with mode {mode}"
            )

            writer: DataFrameWriter = df.write.format("iceberg").mode(mode)
            writer = writer.option(key="overwrite-mode", value=overwrite_mode).options(
                key="merge-schema", value=merge_schema
            )

            self.log_info(
                message=f"Set overwrite-mode to {overwrite_mode} for selective partition overwriting"
            )
            ...
            if partition_by:
                if isinstance(partition_by, str):
                    ...
                    error_msg: str = (
                        f"partition_by must be a string or list of strings, got {type(partition_by)}."
                    )
                ...
                self.log_info(
                    message=f"Set up partitioning on columns: {', '.join(partition_columns)}"
                )

            writer.save(path=iceberg_table)
            self.log_info(
                message=f"Data successfully written to Iceberg table: {iceberg_table}."
            )
    ...
    def write_to_catalog(
        ...
        self.log_debug(
            f"write_to_dynamic get data type {type(data)} db:{db_name} table:{table_name}"
        )
        ...
        try:
            if isinstance(data, DataFrame):
                dynamic_frame: "DynamicFrame" = DynamicFrame.fromDF(
                    data, self.glue_context, "dynamic_frame"
                )
            ...
            self.log_info(
                message=f"Data successfully uploaded Data Catalog: {db_name}.{table_name}"
            )
        ...
        try:
            if isinstance(df, DataFrame):
                df = DynamicFrame.fromDF(
                    dataframe=df, glue_ctx=self.glue_context, name="dynamic_frame"
                )
    ...
    def load_from_s3(
        ...
        try:
            ...
            connection_options: Dict = {"paths": [source_path], "recurse": recurse}
